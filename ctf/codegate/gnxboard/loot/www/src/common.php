...
// gnxboard: to prevent inject g4_path
$dirname = dirname(__FILE__).'/';
include_once("$dirname/lib/constant.php");  // 상수 정의
include_once("$dirname/config.php");  // 설정 파일
include_once("$dirname/lib/common.lib.php"); // 공통 라이브러리
...
if (file_exists("$dirname/$dbconfig_file"))
{
    if (is_dir("$dirname/install")) die("<meta http-equiv='content-type' content='text/html; charset=$g4[charset]'><script type='text/javascript'> alert('install 디렉토리를 삭제하여야 정상 실행됩니다.'); </script>");

    include_once("$dirname/$dbconfig_file");
...
session_save_path("{$dirname}data/session");
...
if (isset($board['bo_skin']))
    $board_skin_path = "{$dirname}/skin/board/{$board['bo_skin']}"; // 게시판 스킨 경로
...
$tmp = dir("$dirname/extend");
while ($entry = $tmp->read()) {
    // php 파일만 include 함
    if (preg_match("/(\.php)$/i", $entry))
        include_once("$dirname/extend/$entry");
}
?>
